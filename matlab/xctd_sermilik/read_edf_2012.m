% read_edf_2012.m
% this script is a misnomer as I don't have the SF 2012 edf files -- rather
...
load('raw mat/XCTD_AUG2012_raw.mat');

% this file begins with 4 casts from Kanger...
Zxctd = Zxctd(:,5:end);
Txctd = Txctd(:,5:end);
Sxctd = Sxctd(:,5:end);

nx = 7; % number of xctd casts
...
time(cnum(1)) = datenum(2012,09,14,19,05,0); % time in UTC
time(cnum(2)) = datenum(2012,09,14,19,11,0);
time(cnum(3)) = datenum(2012,09,14,19,17,0);
time(cnum(4)) = datenum(2012,09,14,19,23,0);
time(cnum(5)) = datenum(2012,09,14,19,29,0);
time(cnum(6)) = datenum(2012,09,14,19,36,0);
time(cnum(7)) = datenum(2012,09,14,19,41,0);
...
save('Sermilik2012_XCTD.mat','depth','time','lat','lon','temp','sal','pres')

%% create netCDF


outfile = 'Sermilik2012_XCTD.nc' 
delete(outfile)
